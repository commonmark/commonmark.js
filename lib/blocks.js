...
var incorporateLine = function(ln) {
    ...
    while (true) {
        ...
        if (indent < CODE_INDENT && matchAt(reMaybeSpecial, ln, next_nonspace) === -1) {
            break;
        }
        ...
        if (indent >= CODE_INDENT) {
            ...
            if (this.tip.type !== 'Paragraph' && !blank) {
                this.offset -= (indent - CODE_INDENT);
                allClosed = allClosed ||
   ...
   // First check for a lazy paragraph continuation:
    if (!allClosed && !blank &&
        ...
        this.tip._string_content.length > 0) {
