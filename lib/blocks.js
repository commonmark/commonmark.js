...
var blockStarts = [
    // indented code block
    function(parser) {
        if (parser.nextNonspace - parser.offset >= CODE_INDENT) {
    ...
    // block quote
    function(parser) {
        if (parser.currentLine.charAt(parser.nextNonspace) === '>') {
            parser.offset = parser.nextNonspace + 1;
            // optional following space
            if (parser.currentLine.charAt(parser.offset) === ' ') {
                parser.offset++;
            }
            parser.closeUnmatchedBlocks();
    ...
    function(parser) {
        ...
        if ((match = parser.currentLine.slice(parser.nextNonspace).match(reCodeFence))) {
            ...
            container._fenceOffset = parser.nextNonspace - parser.offset;
    ...
    // HTML block
    function(parser) {
        if (matchAt(reHtmlBlockOpen, parser.currentLine, parser.nextNonspace) !== -1) {
            parser.closeUnmatchedBlocks();
    ...
    // hrule
    function(parser) {
        if (matchAt(reHrule, parser.currentLine, parser.nextNonspace) !== -1) {
            parser.closeUnmatchedBlocks();
    ...
    function(parser, container) {
        var data;
        if ((data = parseListMarker(parser.currentLine, parser.nextNonspace,
                                    parser.nextNonspace - parser.offset))) {
            parser.closeUnmatchedBlocks();
...
var incorporateLine = function(ln) {
    ...
    var matchedLeaf = container.type === 'CodeBlock' || container.type === 'HtmlBlock';
