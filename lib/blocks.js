...
var blocks = {
    ...
    BlockQuote: {
        continue: function(parser) {
            ...
            if (parser.indent <= 3 &&
                ln.charCodeAt(parser.nextNonspace) === C_GREATERTHAN) {
...
var blockStarts = [
    ...
    // block quote
    function(parser) {
        if (parser.currentLine.charCodeAt(parser.nextNonspace) === C_GREATERTHAN) {
            parser.offset = parser.nextNonspace + 1;
            // optional following space
            if (parser.currentLine.charCodeAt(parser.offset) === C_SPACE) {
                parser.offset++;
            }
            parser.closeUnmatchedBlocks();
