...
var blocks = {
    ...
    BlockQuote: {
        continue: function(parser) {
            ...
            if (parser.nextNonspace - parser.offset <= 3 &&
                ln.charAt(parser.nextNonspace) === '>') {
    ...
    Item: {
        continue: function(parser, container) {
            if (parser.nextNonspace === parser.currentLine.length) { // blank
                parser.offset = parser.nextNonspace;
            } else if (parser.nextNonspace - parser.offset >=
    ...
    CodeBlock: {
        continue: function(parser, container) {
            var ln = parser.currentLine;
            var indent = parser.nextNonspace - parser.offset;
            if (container._isFenced) { // fenced
                ...
                } else if (parser.nextNonspace === ln.length) { // blank
                    parser.offset = parser.nextNonspace;
                } else {
                    return 1;
                }
    ...
    HtmlBlock: {
        continue: function(parser) {
            return (parser.nextNonspace === parser.currentLine.length ? 1 : 0);
        },
        finalize: function(parser, block) {
            block._literal = block._string_content.replace(/(\n *)+$/, '');
            block._string_content = null; // allow GC
...
var findNextNonspace = function() {
    var ln = this.currentLine;
    var match = matchAt(reNonSpace, ln, this.offset);
    if (match === -1) {
        this.nextNonspace = ln.length;
    } else {
        this.nextNonspace = match;
    }
};
...
var incorporateLine = function(ln) {
    ...
    blank = this.nextNonspace === ln.length;
    ...
    while ((t = container.type) && !(t === 'CodeBlock' || t === 'HtmlBlock')) {
        ...
        this.findNextNonspace();
        blank = this.nextNonspace === ln.length;
        indent = this.nextNonspace - this.offset;

        // this is a little performance optimization:
        if (indent < CODE_INDENT && !reMaybeSpecial.test(ln.slice(this.nextNonspace))) {
        ...
        if (indent >= CODE_INDENT) {
        ...
        } else if (ln.charAt(this.nextNonspace) === '>') {
            // blockquote
            this.offset = this.nextNonspace + 1;
            // optional following space
            if (ln.charAt(this.offset) === ' ') {
                this.offset++;
            }
            this.closeUnmatchedBlocks();
            container = this.addChild('BlockQuote', this.nextNonspace);

        } else if ((match = ln.slice(this.nextNonspace).match(reATXHeaderMarker))) {
            // ATX header
            this.offset = this.nextNonspace + match[0].length;
            this.closeUnmatchedBlocks();
            container = this.addChild('Header', this.nextNonspace);
        ...
        } else if ((match = ln.slice(this.nextNonspace).match(reCodeFence))) {
            // fenced code block
            var fenceLength = match[0].length;
            this.closeUnmatchedBlocks();
            container = this.addChild('CodeBlock', this.nextNonspace);
            container._isFenced = true;
            container._fenceLength = fenceLength;
            container._fenceChar = match[0][0];
            container._fenceOffset = indent;
            this.offset = this.nextNonspace + fenceLength;

        } else if (matchAt(reHtmlBlockOpen, ln, this.nextNonspace) !== -1) {
            // html block
            this.closeUnmatchedBlocks();
            ...
            break;
                   ...
                   ((match = ln.slice(this.nextNonspace).match(reSetextHeaderLine)))) {
            // setext header line
            this.closeUnmatchedBlocks();
        ...
        } else if (matchAt(reHrule, ln, this.nextNonspace) !== -1) {
            // hrule
            this.closeUnmatchedBlocks();
            container = this.addChild('HorizontalRule', this.nextNonspace);
            this.offset = ln.length;
            break;

        } else if ((data = parseListMarker(ln, this.nextNonspace, indent))) {
            // list item
            this.closeUnmatchedBlocks();
            this.offset = this.nextNonspace + data.padding;

            // add the list if needed
            if (t !== 'List' ||
                !(listsMatch(container._listData, data))) {
                container = this.addChild('List', this.nextNonspace);
                container._listData = data;
            }

            // add the list item
            container = this.addChild('Item', this.nextNonspace);
            container._listData = data;
